$(function()
{
    ...
    var street_id = $('#pre_street_id').val();
	if (province_id) {
		//显示城市列表
    ...
    if (area_id) {
        change_street(area_id, street_id);
    }
...
function change_city(province_id, city_id)
{
    ...
    $('#street_id').html('<option value="0">选择街道</option>');
    $('#streetSelP').css('display', 'none');
...
function change_street(area_id, street_id)
{
    $.post('/Area/get_street_list', {
        "area_id": area_id
    }, function(data) {
        if (data == null) {
            $('#street_id').html('<option value="0">选择街道</option>');
            ...
            $('#streetSelP').css('display', '');
        }

        var result = eval(data);
        var length = result.length;
        var str = '<option value="0">选择街道</option>';
        for (i = 0; i < length; i++) {
            str += '<option value="' + result[i]['street_id'] + '">' + result[i]['street_name'] + '</option>';
        }

        $('#street_id').html(str);
        if (street_id)
		{
            $('#street_id option[value=' + street_id + ']').attr('selected', 'selected');
            $('#s_mark').html($('#street_id').find('option:selected').text()); //更改文本显示
        }
    }, 'json');
}
//搜索写字楼表单
			...
			if(d) {
				if(d != 'failure')
				{
					//返回成功，代码写这里;1添加2编辑
						...
						opt = opt == 1 ? '添加' : '修改';
						alert('恭喜您，收货地址' + opt + '成功！','/FrontAddress/address_list');
						window.location.href = "/FrontAddress/address_list";
