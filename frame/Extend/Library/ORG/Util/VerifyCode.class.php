 ...
 *
 * @editor <me@filowlee.com>
 * @copyright Apache
 * 于2013年12月14日  修改本文件，美化了生成的验证码，并适配于ThinkPHP
 */
class VerifyCode {
    /**
     * 验证码的session的下标
    ...
    public static $expire = 100;     // 验证码过期时间（s）
    /**
    ...
    public static $fontSize = 14;     // 验证码字体大小(px)
    public static $useCurve = false;   // 是否画混淆曲线
    public static $useNoise = false;   // 是否添加杂点    
    public static $imageH = 40;        // 验证码图片宽
    public static $imageL = 110;        // 验证码图片长
    ...
    public static function createImage() {
        ...
        // 验证码使用随机字体 
        $ttf = APP_PATH . 'Public/font/vcode/1.ttf';  
        if (self::$useNoise) {
            // 绘杂点
            self::_writeNoise();
        } 
        ...
        for ($i = 0; $i<self::$length; $i++) {
            ...
            $codeNX += mt_rand(self::$fontSize*1.4, self::$fontSize*1.6);
            // 写一个验证码字符
            imagettftext(self::$_image, self::$fontSize, mt_rand(-10, 30), $codeNX, self::$fontSize*1.5, self::$_color, $ttf, $code[$i]);
        }
        // 保存验证码
        $str = join('', $code);
        $str = strtoupper($str);
        $_SESSION[self::$seKey] = md5($str);
