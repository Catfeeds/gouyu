		...
		if($jsondecode != null)
		{
			if (property_exists($jsondecode, "errcode"))
			{
				if ($jsondecode->{"errcode"} == "40030")
				{
					...
					$url = 'https://open.weixin.qq.com/connect/oauth2/authorize?appid=' . $this->appid . '&redirect_uri=' . urlencode($redirect_uri) .'&response_type=code&scope=snsapi_userinfo&state=1#wechat_redirect';
					redirect($url);
				}
			}
			if(property_exists($jsondecode, "openid"))
			{
				...
				if (!$user_info)
				{
					...
					$arr['area_id'] = 0;
					$arr['parent_id'] = intval($rec_user_id);
...
#echo $user_model->getLastSql();
                    ...
                    //首次进入的系统奖励
                    //$award = $GLOBALS['config_info']['REGISTER_AWARD'];
                    //D('Account')->addAccount($user_id, AccountModel::REGISTER_GIVE , $award, '新用户注册赠送');
