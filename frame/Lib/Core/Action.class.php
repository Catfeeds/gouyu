...
abstract class Action {
   ...
   /**
    ...
    protected function display($templateFile='',$charset='',$contentType='',$content='',$prefix='') {
		$templateFile2 = $this->_get_template_file();
log_file($templateFile2, 'file_path', true);
		$templateFile = $templateFile2 ? $templateFile2 : $templateFile;
		#dump($templateFile);
		#die;
        $this->initView();
    ...
    public function __call($method,$args) {
        if( 0 === strcasecmp($method,ACTION_NAME.C('ACTION_SUFFIX'))) {
            ...
            if(!isset($args[0])) { // 获取全局变量
                ...
                $data       =	$input[$args[0]];
                $filters    =   isset($args[2])?$args[2]:C('DEFAULT_FILTER');
     ...
     * @param string $dom_name 报错的dom名称 
     * @return void
     */
    protected function error($message,$jumpUrl='',$ajax=false, $dom_name = '') {
        $this->dispatchJump($message,0,$jumpUrl,$ajax, $dom_name);
    }
     ...
     * @return void
     */
    private function dispatchJump($message,$status=1,$jumpUrl='',$ajax=false, $dom_name = '') {
        if(true === $ajax || IS_AJAX) {// AJAX提交
            $data           =   is_array($ajax)?$ajax:array();
            $data['info']   =   $message;
            $data['status'] =   $status;
            $data['url']    =   $jumpUrl;
            $data['dom_name']    =   $dom_name;
            $this->ajaxReturn($data);
        }
        if(is_int($ajax)) $this->assign('waitSecond',$ajax);
        if(!empty($jumpUrl)) $this->assign('jumpUrl',$jumpUrl);
        if(!empty($jumpUrl)) $this->assign('dom_name',$dom_name);
        // 提示标题
        ...
        if($status) { //发送成功信息
			...
			if (is_a($this, 'AcpAction'))
			{
				$this->display(C('TMPL_ACTION_SUCCESS'));
			}
			else
			{
				$this->display(C('COMM_TMPL_ACTION_SUCCESS'));
			}
            exit;
        }else{
			...
			{
				$this->display(C('COMM_TMPL_ACTION_ERROR'));
			}
            // 中止执行  避免出错后继续执行
            exit ;
        }
   ...
   /**
    ...
    public function _get_template_file($action_name = '') {
		...
		$page_id = intval($templet_info[$action_name]);
		if (empty($templet_info) || !$page_id)
		{
			return '';
		}
		...
		$page_info = $page_obj->getPageInfo('page_id = ' . $page_id);
		//若当前请求页面不在页面模板表中，直接退出
		if (!$page_info)
		{
			log_file('找不到page_id为' . $page_id . '的模板', 'templet');
			return '';
		}

		//根目录
		$root_path = dirname(dirname(dirname(dirname(__FILE__))));
		$file_path = $root_path . '/Tpl' . '/Public/' . $action_name . '/' . $page_info['file_name'] . '/' . 'index.html';
		#echo $file_path . "<br>";
		#die;
     ...
     * @access private
     */
    public function _get_template_rel_file($action_name = '') {
        global $install_info;
        $action_name = $action_name ? $action_name : ACTION_NAME;
        ...
        //根目录
        $root_path = dirname(dirname(dirname(dirname(__FILE__))));
        $file_path = '/Tpl' . DS . '/Public/' . $action_name . '/' . $page_info['file_name'] . '/index.html';
        #dump($file_path);
        #return file_exists($file_path) ? $file_path : '';
        return $file_path;
    }
