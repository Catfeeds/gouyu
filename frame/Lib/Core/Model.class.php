...
class Model {
	...
	/**
	 * 查询多条数据时，从第$start条开始取
	 */
    protected $start = 0;

	/**
	 * 查询多条数据分页返回时单页的数据量，默认为15
	 */
    protected $limit = 15;
    ...
    public function select($options=array()) {
		//所有从Az查询的操作都从缓存中取
		if (is_a($this, 'AzModel'))
		{
			$this->cache();
		}

		/*套装信息查询从缓存中取*/
		if (is_a($this, 'TemplateModel'))
		{
			$this->cache();
		}
		if (is_a($this, 'PagePackageLayoutModel'))
		{
			$this->cache();
		}
		if (is_a($this, 'PagePackageLayoutModel'))
		...
		//所有本地查询的操作都从缓存中取
		if (is_a($this, 'ModuleBaseModel'))
		{
			$this->cache();
		}

		//排序处理
		if (!isset($this->options['order']) && $this->limit > 1)
		{
			if (is_a($this, 'ItemBaseModel') || is_a($this, 'OrderBaseModel'))
			{
				$this->options['order'] = 'addtime DESC';
			}
		}
    ...
    public function limit($offset = null,$length=null){
		...
		if (is_null($offset))
		{
			$this->options['limit'] = $this->getStart() . ', ' . $this->getLimit();
		}
	...
	public function setStart($start)
	{
		...
		{
			trigger_error(__CLASS__ . ', ' . __FUNCTION__ . ', 无效的参数start');
		}
    }
	...
	public function setLimit($limit)
	{
		...
		{
			trigger_error(__CLASS__ . ', ' . __FUNCTION__ . ', 无效的参数limit');
		}
    }
