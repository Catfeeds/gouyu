...
log_file('file_data = ' . json_encode($_FILES));
		...
		$token = $api_obj->generateSign($data);
		$data['token'] = $token;
		#echo $token;die;
		#echo "<pre>";
		#print_r($data);
...
log_file('upfile-api: request = ' . json_encode($_REQUEST));
log_file('upfile-api: upfile1 = ' . json_encode($_FILES));
		if ($submit == 'submit')
		{
log_file('upfile-api: upfile = ' . json_encode($_FILES));
			$path = uploadImage($_FILES['upfile'], '/app');
	...
	function parseRequest()
	{
		...
		$this->api_type = ucfirst($requests[1]) . 'ApiModel';
	 ...
	 */
	function checkParamsValid($params = array())
	{
		$new_params = array();
