...
* @Date 2014-09-04
 */
class AcpOrderAction extends AcpAction
{
    /**
     * 初始化
     * @author 姜伟
...
trace($order_list);
        ...
        if ($opt == 'EXCHANGE') {
            $this->display(APP_PATH . 'Tpl/AcpOrder/get_exchange_order_list.html');
        } else {
            $this->display(APP_PATH . 'Tpl/AcpOrder/get_order_list.html');
        }
	}
	...
	{
		$this->order_list('isuse = 1 AND need_deliever = 1 AND order_status = ' . OrderModel::PAYED, '待发货的订单', 'PAYED');
	}
	 ...
	 * @todo 接收表单数据，验证数据有效性，组成入参，调用订单模型的方法获取订单列表
	 */
	public function get_pre_exchange_order_list()
	{
		$this->order_list('isuse = 1 AND need_deliever = 0 AND  order_status = ' . OrderModel::PAYED, '待领取的订单', 'EXCHANGE');
	}
	...
	{
		$this->order_list('need_deliever = 1 AND isuse = 1 AND order_status = ' . OrderModel::DELIVERED, '已发货的订单', 'DELIVERED');
	}
	public function get_exchanged_order_list()
	{
		$this->order_list('need_deliever = 0 AND isuse = 1 AND order_status = ' . OrderModel::DELIVERED, '已发货的订单', 'EXCHANGE');
	}
	...
	public function order_detail()
	{
        ...
        # wsq added
        # 获取加盟商资料
		$fields    = 'username, realname, nickname, mobile, email, qq, id';
...
#		$this->assign('gift_list', $gift_list);
	...
	private function refund_change_order_list($where, $head_title, $opt)
	{
		...
		$this->assign('opt', $opt);
		$this->assign('head_title', $head_title);
		$this->display(APP_PATH . 'Tpl/AcpOrder/get_refund_change_order_list.html');
	}
	...
	public function order_refund_change_apply_detail()
	{
		...
		$order_id = intval($order_id);
		$redirect = $this->_get('redirect');
		$redirect = $redirect ? url_jiemi($redirect) : U('/AcpOrder/get_pre_refund_order_list');
		if (!$order_id)
		{
    ...
    public function order_stat_by_day() 
    {
		...
		//获取订单统计数据
		$order_obj = new OrderModel();
		$order_stat_list = $order_obj->field('DATE_FORMAT(FROM_UNIXTIME(addtime), "%H") AS hour, SUM(pay_amount) AS total_amount')->where('addtime >= ' . $start_time . ' AND addtime <= ' . $end_time)->group('hour')->order('addtime DESC')->select();
    ...
    public function order_stat_by_month() 
    {
		...
		//获取订单统计数据
		$order_obj = new OrderModel();
		$order_stat_list = $order_obj->field('DATE_FORMAT(FROM_UNIXTIME(addtime), "%d") AS day, SUM(pay_amount) AS total_amount, SUM(pay_amount - express_fee) AS profit')->where('addtime >= ' . $start_time . ' AND addtime <= ' . $end_time)->group('day')->order('addtime DESC')->select();
    ...
    public function order_stat_by_year() 
    {
		...
		//获取订单统计数据
		$order_obj = new OrderModel();
		$order_stat_list = $order_obj->field('DATE_FORMAT(FROM_UNIXTIME(addtime), "%m") AS month, SUM(pay_amount) AS total_amount, SUM(pay_amount -  express_fee) AS profit')->where('addtime >= ' . $start_time . ' AND addtime <= ' . $end_time)->group('month')->order('addtime DESC')->select();
        ...
        if ($type) {
            if ($type == 'small_area') {
                if (!$area_id && !$city_id && !$province_id) 
                    $this->error('请选择省市区', U('/AcpOrder/order_stat_by_area'));
                ...
                if ($big_area_id<0) 
                    $this->error('请选择大区', U('/AcpOrder/order_stat_by_area'));
    ...
    public function deliver_order()
    {
        ...
        if ($order_id) {
            ...
            //提醒发货
            $info = $order_obj->getOrderInfo('order_sn, user_id');
            $order_item_obj = new OrderItemModel($order_id);
            $item_list = $order_item_obj->getOrderItemList();
            $item_name  = '';
            ...
            $msg = array(
                'first'		=> '尊敬的用户，您的商品发货啦！',
                'keyword1'	=> '商城兑换商品',
                'keyword2'	=> $info['order_sn'],
                'keyword3'	=> $item_name,
                'url'		=> 'http://' . $_SERVER['HTTP_HOST'] . '/FrontUser/my_exchange.html',
            );
    ...
    //获取团购订单
    function group_order_list($where, $head_title, $opt)
	{
		$where .= $this->get_search_condition();
		$order_obj = new OrderModel();

        $joins = 'tp_group_buy AS g ON g.group_buy_id = o.group_buy_id';

        //分页处理
        import('ORG.Util.Pagelist');
        //$count = $order_obj->getOrderNum($where);
        $count = $order_obj->alias('o')
            ->join($joins)
            ->where($where)
            ->count();
		...
		//$order_list = $order_obj->getOrderList('', $where, ' addtime DESC');
        $fields = 'order_id, order_sn, pay_amount, total_amount, order_status, user_id, user_address_id, express_company_id, express_number, payway, weight, o.addtime, pay_time, start_delivery_time, confirm_time, o.isuse, o.group_buy_id, is_group_buy, group_name, people_limit, end_time, start_time, status';
        $order_list = $order_obj->alias('o')
            ->join($joins)
            ->field($fields)
            ->where($where)
		...
		$this->assign('opt', $opt);
		$this->assign('head_title', $head_title);
		$this->display(APP_PATH . 'Tpl/AcpOrder/get_group_order_list.html');
	}

    //group
    public function get_group_payed_order_list()
    {
		$this->group_order_list('o.is_group_buy = 1 AND o.isuse = 1 AND o.order_status = ' . OrderModel::PAYED . ' AND g.status = 1 AND g.end_time > ' . NOW_TIME, '待发货的订单', 'PAYED');
    }

    public function get_group_pre_deliver_order_list()
	{
		$this->group_order_list('o.is_group_buy = 1 AND o.isuse = 1 AND o.order_status = ' . OrderModel::PAYED . ' AND g.status = 2', '待发货的订单', 'PAYED_FINISH');
	}
    public function get_group_delivered_order_list()
    {
		$this->group_order_list('o.is_group_buy = 1 AND o.isuse = 1 AND o.order_status = ' . OrderModel::DELIVERED . ' AND g.status = 2', '已发货的订单', 'DELIVERED');
	}
    public function get_group_pre_refund_order_list()
    {
		$this->group_order_list('o.is_group_buy = 1 AND o.isuse = 1 AND o.order_status = ' . OrderModel::PAYED. ' AND g.status = 1 AND g.end_time < ' . NOW_TIME, '待退款的订单', 'PRE_REFUND');
    }
    public function get_group_refunded_order_done_list()
    {
		$this->group_order_list('o.is_group_buy = 1 AND o.isuse = 1 AND o.order_status = ' . OrderModel::REFUNDED, '已退款的订单', 'REFUNDED');
		//$this->refund_change_order_list('operate_type = 1 AND tp_item_refund_change.isuse = 1', '所有退款订单', 'REFUNDED');
    }
}
