...
class AcpIndexAdsAction extends AcpAction
{
	public function AcpIndexAdsAction()
	{
		parent::_initialize();
	}
     ...
     * @todo 获取首页广告图片列表，公共方法
     */
	function index_ads_list($where, $head_title, $opt)
	{
        ...
        $count = $index_ads_obj->getIndexAdsNum($where);
        $Page = new Pagelist($count,C('PER_PAGE_NUM'));
		$index_ads_obj->setStart($Page->firstRow);
        $index_ads_obj->setLimit($Page->listRows);
        $show = $Page->show();
		$this->assign('show', $show);

		$index_ads_list = $index_ads_obj->getIndexAdsList('', $where, ' serial ASC');
		$index_ads_list = $index_ads_obj->getListData($index_ads_list);
		$this->assign('index_ads_list', $index_ads_list);
		#echo "<pre>"; print_r($index_ads_list);die;
		#echo "<pre>";
		#print_r($index_ads_list);
		#echo "</pre>";
		#echo $index_ads_obj->getLastSql();

		$this->assign('opt', $opt);
		$this->assign('head_title', $head_title);
		#$this->display(APP_PATH . 'Tpl/AcpIndexAds/get_index_ads_list.html');
	}

	public function get_index_ads_list()
	{
		$this->index_ads_list('1 = 1', '首页首页广告图片列表');
		$this->display();
    ...
    public function add_index_ads()
    {
		...
		if($submit == 'submit')				//执行添加操作
		{
			...
			$index_ads_obj = new IndexAdsModel();
			$index_ads_id = $index_ads_obj->addIndexAds($data,1);
			#echo "<pre>";
			#print_r($data);
			#die;
			if ($index_ads_id)
			{
				$this->success('恭喜您，首页广告图片添加成功','/AcpIndexAds/get_index_ads_list');
			}
			else
			{
		...
		$this->assign('head_title','添加首页广告图片');
		$this->display();
	}
     ...
     * 
     */
    public function edit_index_ads()
    {
		$redirect = $this->_get('redirect');
		$index_ads_id = intval($this->_get('index_ads_id'));
		$index_ads_obj = new IndexAdsModel();
		$index_ads_info = $index_ads_obj->getIndexAdsInfo('index_ads_id = ' . $index_ads_id, '');
		#echo $index_ads_obj->getLastSql();
		#echo "<pre>";
		#print_r($index_ads_info);
		#die;

		if (!$index_ads_info)
		{
			$this->error('抱歉，轮播图位不存在', U('/AcpIndexAds/get_index_ads_list'));
		}
		...
		if($submit == 'submit')				//执行添加操作
		{
            ...
            // dump($data);die;
			if($index_ads_info['pic'] != $pic) {
            	$arr_img=$this->_resizeImg(APP_PATH . ltrim($pic, '/'));
                // dump($arr_img);
            }
            // die;

			$index_ads_obj = new IndexAdsModel($index_ads_id);
			$success = $index_ads_obj->editIndexAds($data);
            // echo $index_ads_obj->getLastSql();die;
			if ($success)
			{
				$this->success('恭喜您，首页广告图片修改成功');
			}
			else
			{
        ...
        // dump($index_ads_info);die;
		foreach ($index_ads_info AS $k => $v)
		{
			if ($k == 'pic')
			{
			...
			if ($k == 'link')
			{
				$this->assign('index_ads_link', $v);
			}
			$this->assign($k, $v);
		}

		$this->assign('head_title','修改首页广告图片');
		$this->display();
	}

	//删除轮播图
	public function del_index_ads()
    {
		$id        = intval($this->_post('id'));
		if ($id)
		{

            $index_ads_obj = new IndexAdsModel($id);
            $success = $index_ads_obj->delIndexAds($id);
    ...
    public function add_adv()
    {
		...
		if($submit == 'submit')				//执行添加操作
		{
			...
			if ($adv_id)
			{
				$this->success('恭喜您，图片添加成功','/AcpIndexAds/top_adv_list');
			}
			else
			{
				$this->success('抱歉，添加失败');
    ...
    public function edit_adv()
    {
		...
		if (!$adv_info)
		{
			$this->error('抱歉，广告图位不存在', U('/AcpIndexAds/top_adv_list'));
		}
