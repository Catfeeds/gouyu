...
class AcpUserAction extends AcpAction
{
    ...
    {
        $this->user_list('role_type = 3 AND is_extend_user = 1', '推广员列表', 'extend_user_b');
    }
    ...
    public function extend_user()
    {
        ...
        $this->user_list('role_type = 3 AND parent_id = ' . $parent_id, '推广列表', 'extend_user');
    }
    ...
    public function user_detail()
    {
        ...
        //调用用户模型的getUserInfo获取用户信息
        $user_obj = new UserModel($user_id);
        $user_info = $user_obj->getUserInfo('username, nickname, realname, email, mobile, address, province_id, city_id, area_id, big_area_id, consumed_money, left_money, left_integral, user_rank_id, reg_time, store_sn, user_type, qq, is_enable');
     ...
     * wzg
     */
    public function recharge_back_most_by_week()
    {
        $recently_week = $this->recent_week();

        $recharge_back_obj = new RechargeBackModel();
        $arr = array();
        foreach ($recently_week AS $k=>$v) {
            $info = $recharge_back_obj->field('sum(gain_money) AS user_gain, user_id')
                ->where('addtime >= ' . $v['start_week'] . ' AND addtime <= ' . $v['end_week'])
                ->order('user_gain DESC')
                ->group('user_id')
                ->find();
            ...
            if ($info) {
                ...
                $user_info = $user_obj->getUserInfo('headimgurl, nickname, id');
                $info['headimgurl'] = $user_info['headimgurl'];
                $info['nickname'] = $user_info['nickname'];
                $info['id'] = $user_info['id'];
                $info['start_week'] = $v['start_week'];
                $info['end_week'] = $v['end_week'];
        ...
        $this->assign('head_title', '推广收益周最佳');
        $this->display();
    }

    //红黑榜
    function get_week_times()
    {
        $list = D('Treasure')->calWeekTimes(true);
        foreach ($list AS $k=>$v) {
            $list[$k]['name'] = M('Users')->where('user_id = ' . $v[0])->getField('nickname');
        }
        $this->assign('week_times', $list);
        $this->display();
    }
}
?>
