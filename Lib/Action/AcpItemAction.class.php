...
class AcpItemAction extends AcpAction {
	 ...
	 * @todo 获取商品列表，公共方法
	 */
	function item_list($where, $head_title, $opt)
	{
     ...
     * @todo 获取库存警报的商品列表
     */
    function get_alarm_item_list()
    {
        $this->item_list('isuse = 1 AND stock <= stock_alarm AND stock > 0', '库存警报的' . C('ITEM_NAME') . '列表', 'alarm');
    }
     ...
     * @todo 获取售罄的商品列表
     */
    function get_sold_out_item_list()
    {
        $this->item_list('isuse = 1 AND stock <= 0', '售罄的' . C('ITEM_NAME') . '列表', 'sold_out');
    }
	...
	public function edit_item()
	{
		...
		if ($act == 'edit')
		{
			...
			$arr = array(
				...
				'is_real'     	=> I('is_real'),
				'use_hero'     	=> I('use_hero', '', 'strip_tags'),
            ...
            if ($has_sku) {			//如果开启sku
            	...
            	$sku0_arr 		= $this->_post('sku0',array());		//系统规定了每一件商品最多只有2个规格属性。数组长度与$sku_ids等同
     ...
     * @todo 商品设置列表，公共方法
     */
    function item_set_list($where, $title, $action_name)
    {
        if ($where != 'no_item') {
            $where .= $this->get_search_condition();
            $item_obj = new ItemModel();
