...
class AcpNoticeAction extends AcpAction {
     ...
     * @author 陆宇峰
     * @return void
     * @todo 删除notice_sort表的数据，删除前确定是否有该分类下的公告，提示无法删除，请先调整公告分类。表内ID20以内的数据不能删除
     */
	...
	public function list_notice()
	{
		...
		$_table = $generalNotice->getTableName();
		if($act == 'submit')
		{
			$keyword 	   = $this->_get('keyword');
			...
			if($sortId)
			{
				$conditions[] = $_table . '.notice_sort_id=' . $sortId;
			}
			...
			$this->assign('notice_category_option_selected', $sortId);
		}
		$rows = 15;
		$generalNoticeList = $generalNotice->getGeneralNoticeListPage($where, $rows);
	//	echo "<pre>";
	//	print_r($generalNoticeList);die;
		
		if($generalNoticeList && is_array($generalNoticeList))
		{
			$pagination = array_pop($generalNoticeList);
			foreach($generalNoticeList as $key => $val)
			{
				$generalNoticeList[$key]['title'] = mbSubStr($val['title'], 25);
		...
		$noticeCategoryList = $noticeCategory->getNoticeCategoryList('', '', $fields, 'isuse = 1');
		if($noticeCategoryList && is_array($noticeCategoryList))
		{
			foreach($noticeCategoryList as $key => $val)
			{
				$noticeCategoryOptions[$val['notice_sort_id']] = $val['notice_sort_name'];
			}
			$this->assign('notice_category_options', $noticeCategoryOptions);
		}
     ...
     * @author 陆宇峰
     * @return void
     * @todo 添加公告，tp_notice表，isuse默认1，az_notice_id=0
	...
	public function add_notice()
	{
		...
		if($act == 'submit')
		{
			...
			$generalNotice = new GeneralNoticeModel();
			if($id = $generalNotice->addNotice($data))
			{
				...
				$this->success('恭喜您，公告添加成功！', '/AcpNotice/list_notice');
			}
			else
			{
				$this->error('对不起，公告添加失败！');
			}
		}
		//分类列表
		$noticeCategory = new NoticeCategoryModel();
		...
		$this->assign('action_title', '公告列表');
		$this->assign('action_src', '/AcpNotice/list_notice');
		$this->assign('head_title', '添加公告');
		$this->display();
	}
	...
	public function edit_notice()
	{
		...
		$generalNotice = new GeneralNoticeModel();
		if(!$generalNotice->getNoticeInfo($id))
		{
			$this->error('无效ID！');
		}
		...
		if($act == 'submit')
		{
			...
			if($generalNotice->saveNotice($id, $data))
			{
				...
				$this->success('恭喜您，公告修改成功！', '/AcpNotice/list_notice');
			}
			else
			{
				$this->error('对不起，公告修改失败！');
			}
		}
		
		$noticeData = $generalNotice->getGeneralNoticeById($id);
	//	echo "<pre>";
     ...
     * @author 陆宇峰
     * @return void
     * @todo 从AZ列出开网店教程的公告，sort_id=1，可以点击下载或者批量下载。保存到本地后，sort_id=1
     */
     ...
     * @author 陆宇峰
     * @return void
     * @todo 从AZ列出网店通用素材的公告，sort_id=2，可以点击下载或者批量下载。保存到本地后，sort_id=2
     */
     ...
     * @author 陆宇峰
     * @return void
     * @todo 从AZ列出常用软件的公告，sort_id=3，可以点击下载或者批量下载。保存到本地后，sort_id=3
     */
     ...
     * @author 陆宇峰
     * @return void
     * @todo 使用az的数据库连接类，列出az上的notice表。支持公告名搜索，az的sort_id=4
     */
     ...
     * @author 陆宇峰
     * @return void
     * @todo 从AZ拉取 SEO知识站关的公告，不用保存本地，直接读。AZnotice表，sort_id = 5的公告，
     */
