...
class AcpArticleAction extends AcpAction {
    ...
    {
        return array(
            GeneralArticleModel::ROLL_RULE => 'Roll规则',
            GeneralArticleModel::EXCHANGE_RULE => '兑换规则',
            GeneralArticleModel::TREASURE_RULE => '夺宝规则',
            GeneralArticleModel::TREASURE_OPEN_RULE => '夺宝开奖规则',
            GeneralArticleModel::GUESS_RULE => '竞猜规则',
        );
    }
	/**
     * 文章栏目列表
     ...
     * @author 陆宇峰
     * @return void
     * @todo 删除article_sort表的数据，删除前确定是否有该分类下的文章，提示无法删除，请先调整文章分类。表内ID20以内的数据不能删除
     */
	...
	//	print_r($generalArticleList);die;
		...
		if($generalArticleList && is_array($generalArticleList))
		{
			...
			foreach($generalArticleList as $key => $val)
			{
                ...
                //如果分类是规则在后面加入什么规则
                if ($val['rule_type']) {
                    $rule_type_name = self::rule_type()[$val['rule_type']];
                    $generalArticleList[$key]['article_sort_name'] = $val['article_sort_name'] . '(' . $rule_type_name . ')';
                }
			}
	...
	public function add_article()
	{
        ...
        //规则分类
        $this->assign('rule_type_list', self::rule_type());
	...
	public function edit_article()
	{
		...
		if($act == 'submit')
		{
			...
			if($generalArticle->saveArticle($id, $data))
			{
				...
				$this->success('恭喜您，文章修改成功！', '/AcpArticle/list_article');
			}
			else
			{
