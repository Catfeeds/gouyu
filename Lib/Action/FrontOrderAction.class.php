...
class FrontOrderAction extends UcpAction
{
	...
	public function order_submit()
	{
        ...
        //$payway_list = $payway_obj->getPaywayList('', ' AND (pay_tag = "wxpay" OR pay_tag = "wallet")');
	...
	{
		$where = 'isuse = 1 AND user_id = ' . intval(session('user_id')) . ' AND order_status = ' . OrderModel::CONFIRMED;
		$where .= $this->get_search_condition();
		$this->get_order_list($where, '全部订单', 'delivered');
	}
	...
	{
		$where = 'isuse = 1 AND user_id = ' . intval(session('user_id')) . ' AND order_status = ' . OrderModel::REFUNDING;
		$where .= $this->get_search_condition();
	...
	public function order_detail()
	{
		...
		try
		{
			$order_info = $order_obj->getOrderInfo('order_id, order_sn, order_status, pay_amount, total_amount, express_fee, addtime, pay_time, confirm_time, user_address_id, payway', ' AND user_id = ' . $user_id);
		}
		catch (Exception $e)
		{
	...
	public function pay_order()
	{
		...
		else
		{
			...
			//商品库存是否足够
			$order_obj = new OrderModel();
			$order_list = $order_obj->getOrderList('order_id', 'order_id = ' . $order_id);
			$item_obj = new ItemModel();
