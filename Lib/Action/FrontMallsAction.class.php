...
class FrontMallAction extends FrontAction{
	...
	public function item_list()
	{
		...
		$item_list = $item_obj->getItemListGroupBySort($class_id, $total);
	...
	public function get_item_list()
	{
		...
		if ($firstRow <= ($total - 1) && $user_id)
		{
			...
			//获取商品列表
			$item_list = $item_obj->getItemListGroupByClass($merchant_id);
		#echo $item_obj->getLastSql();
		#die;	
	...
	public function mall_home()
	{
		...
		$item_list = $item_obj->getItemList('item_id, item_name, mall_price, market_price, base_pic, sales_num', 'isuse = 1 AND stock > 1', 'serial ASC, addtime DESC');	
		$item_list = $item_obj->getListData($item_list);
		$this->assign('item_list', $item_list);
		//商品总数
