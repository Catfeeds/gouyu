...
class AcpAction extends GlobalAction {
	...
	function _initialize() {
        //dump(D('GuessUser')->cal_odds(1000000, 1000000));die;
		parent::_initialize();
		//取得登录用户信息
		...
		if (!$this->login_user) redirect(U('/login/index/redirect/' . $this->cur_url)); //没登录先进登录页面
		...
		#echo "</pre>";
		//验证权限（首页和部分特殊页面在不用验证）
		if (!in_array(ACTION_NAME, array('index', 'set_menu', 'uploadHandler', 'delImage', 'common_article')) && $user_id != 1)
		{
			// edited by wangsongqing 2015-04-18 21:22
            // 过滤AJAX请求，客悦中主要是文章类修改，其他逻辑相关的都是封装到Action Class中；
            // 所有放行Ajax 的ActionClass暂认为是安全的；
            if (stristr(MODULE_NAME, 'ajax') || in_array(MODULE_NAME, array('AcpIntegral'))) {
                ;
	...
	public function index(){
        ...
        $data = array(
            'OS_name' => php_uname('s'),
            'PHP_run_way' => php_sapi_name(),
            'PHP_Version' => PHP_VERSION,
        );
        $data = array_merge($stat, $data);
        $this->assign("data", $data);


        # 运营数据
        $today_num = D('Users')->where('role_type = 3 AND reg_time > ' . strtotime(date('Y-m-d')) )->count();
        $total_num = D('Users')->where('role_type = 4')->count();
        $this->assign('total_num', $total_num);
        $this->assign('today_num', $today_num);

        $order_today_total = D('Order')->where('addtime> ' . strtotime(date('Y-m-d')) )->count(); 
        $order_total = D('Order')->where('')->count(); 

        $this->assign('order_today_total', $order_today_total);
        $this->assign('order_total', $order_total);

        $user_id = session('user_id');
        $user_info = D('Users')->where('user_id='.intval($user_id))->field('username, realname, nickname')->find();
        if ($user_info) {
            $display_name = $user_info['nickname'];
            $display_name = $display_name ? $display_name:$user_info['realname'];
            $display_name = $display_name ? $display_name:$user_info['username'];
        }
        $display_name = NULL;
        $display_name = $display_name ? $display_name: "系统管理员";
