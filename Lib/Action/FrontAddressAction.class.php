...
class FrontAddressAction extends FrontAction{
	...
	public function user_address_list()
	{
		...
		$user_address_list = $user_address_obj->getUserAddressList('user_id,user_address_id, province_id, city_id, area_id, mobile, address, realname', 'isuse = 1 AND user_id = ' . $user_id, 'addtime DESC, use_time DESC');
trace($user_address_list);
	...
	public function add_user_address()
	{
        ...
        if ('add' == $act) {
            ...
            $is_default = I('is_default', 0, 'int');

            $redirect_url = U('/FrontAddress/add_user_address');
            if (!$realname) $this->alert('请填写收货人姓名', $redirect_url);
            if (!$realname || !checkMobile($mobile)) $this->alert('请填写正确的电话', $redirect_url);
            if (!$province_id || !$city_id || !$area_id || !$address) $this->alert('请填写完整收货地址', $redirect_url);
            ...
            $user_address_obj = new UserAddressModel();
            $success = $user_address_obj->add($arr);
            if ($success) {
                if ($is_default) {
                    $user_obj->setUserInfo(array('user_address_id' => $success));
                    $user_obj->saveUserInfo();
                }
                redirect(U('/FrontAddress/user_address_list'));
            } else {
                $this->alert('出错了，请稍后再试', '/FrontAddress/user_address_list');
            }
        }
	...
	function delete_address()
	{
		...
		if ($user_id && is_numeric($user_address_id))
		{
			...
			if ($user_address_info)
			{
				$success = $user_address_obj->delUserAddress($user_address_id, true);
			}
			#echo 'user_id = ' . $user_id;
			#echo $user_address_obj->getLastSql();
			#die;

			echo $success ? 'success' : '删除失败';
			exit;
		}
...
#log_file($user_obj->getLastSql());
			...
			echo is_numeric($success) ? 'success' : '设置失败';
			exit;
		}
