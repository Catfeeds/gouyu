...
class ItemModel extends Model
{
    ...
    public function getListData($item_list)
    {
		...
		foreach ($item_list AS $k => $v)
		{
            ...
            //$item_list[$k]['exchange_num'] = M('OrderItem')->where('item_id = ' . $v['item_id'])->count();
            //vip价
            //if ($user_rank_info) {
    ...
    {
        if (!is_numeric($class_id))   return false;
        return $this->where('isuse = 1  AND class_id = ' . $class_id)->count();
    }
     ...
     */
    public function calculateItemRealPrice($item_id, $sku_id, $number, $user_rank_id = 0, $calculate_wholesale_rule = true)
    {
		// 用户等级
		if (!$user_rank_id)
		{
			...
			$Users   = M('Users');
			$user_rank_id = $Users->where('user_id = ' . $user_id)->getField('user_rank_id');
		}
		$pay_price = 0.00;
		$rank_price = 0.00;
		$discount_rate = 100;
	 ...
	 * @todo 根据分类获取商品列表
	 */
	public function getItemListGroupBySort($class_id, $limit_num = 4)
	{
		require_cache('Common/func_item.php');
		//获取二级分类列表
		...
		{
			$arr = $this->field($fields)->where('isuse = 1 AND stock > 0 AND sort_id = ' . $v['sort_id'])->order($order)->limit($limit)->select();
     ...
     * wzg
     */
    public function getItemListGroupByWhere($where = 'true',  $limit=null)
    {
        $fields = '*';
		$order = 'serial';
        $where .= ' AND isuse = 1 AND stock > 0';
        $item_list = array();
        $arr = $this->field($fields)->where($where)->order($order)->limit($limit)->select();
        foreach ($arr AS $key => $val)
        {
            $arr[$key]['small_img'] = small_img($val['base_pic']);
            $arr[$key]['middle_img'] = middle_img($val['base_pic']);
            //商品的兑换人数
            $arr[$key]['exchange_num'] = M('OrderItem')->where('item_id = ' . $val['item_id'])->count();
        }
