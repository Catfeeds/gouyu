...
class ConfigBaseModel extends Model
{
    ...
    /**
     * 判断当前时间是否可以充值
     * var $start_time 关闭充值的开始时间
     * var $end_time 关闭充值的结束时间
     * var $is_close 是否设置了关闭
     * return true 可以充值， false 不可以
     * todo 如果设置的开始时间小时大于结束时间的小时，则存在跨天操作
     * param $opt 类型
     * opt == 2 商城关闭时间
     * wzg
     */
    public function is_recharge_time($opt = 1)
    {
        if ($opt == 2) {
            $is_close = $GLOBALS['config_info']['IS_EXCHANGE_CLOSE'];
            $start_time = $GLOBALS['config_info']['CLOSE_EXCHANGE_START_TIME'];
            $end_time  = $GLOBALS['config_info']['CLOSE_EXCHANGE_END_TIME'];
        } else {
            $is_close = $GLOBALS['config_info']['IS_CLOSE'];
            $start_time = $GLOBALS['config_info']['CLOSE_START_TIME'];
            $end_time  = $GLOBALS['config_info']['CLOSE_END_TIME'];
        }
        ...
        //开始时间
        $start_his = date('H:i:s', $start_time);
        $start_time = strtotime($ymd . $start_his);

        //结束时间
        $end_his = date('H:i:s', $end_time);
        $end_time = strtotime($ymd . $end_his);
        //判断是否是跨度两天，如果是则在结束时间上加一天
        $s_h = date('H', $start_time);
        $e_h = date('H', $end_time);
        if (intval($s_h) > intval($e_h)) {
            //如果在跨度中的今天部分，则返回false
            $n_h = date('H', NOW_TIME);
            if (intval($n_h) <= intval($e_h)) return false;
    ...
    //是否关闭了网站
    public function is_close_site()
    {
        $is_open = $GLOBALS['config_info']['SYSTEM_OPEN'];
        if (!$is_open) {
            echo $GLOBALS['config_info']['SYSTEM_CLOSE_REASON'];
            die;
        }
    }
}
