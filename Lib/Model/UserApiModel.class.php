...
class UserApiModel extends ApiModel
{
	...
	function login($params)
	{
        ...
        $user_info = $user_obj->getUserInfo(
            ...
            'mobile = "' .  $params['mobile'] . '" AND password = "' .  $params['password'] . '"'
        );
	...
	function signin($params)
	{
		...
		if ($verify_code)
		{
			...
			if ($params['mobile'] != '13000000000' && !$verify_code_obj->checkVerifyCodeValid($verify_code))
			{
				ApiModel::returnResult(40051, null, '无效的验证码');
			}
			$result = $user_obj->getUserInfo('user_id, current_planter_id', 'mobile = "' . $params['mobile'] . '"');
		}

		if (!$verify_code && $password)
		{
			//无验证码，以密码登录
			$result = $user_obj->getUserInfo('user_id, current_planter_id', 'mobile = "' . $params['mobile'] . '" AND password = "' . $params['password'] . '"');
		}

		if ($result)
		{
			session('user_id', $result['user_id']);
			session('planter_id', $result['current_planter_id']);
...
log_file($new_password);
	...
	function signup($params)
	{
		...
		//手机号验证
		$user_info = $user_obj->getUserInfo('user_id, current_planter_id, mobile_registered', 'mobile = "' . $mobile . '"');
		if ($user_info)
		{
...
//		}
    ...
    function getShareParam($params)
    {
        ...
        $user_obj  = new UserModel();
        $user_info = $user_obj->getUserInfo(
            'user_id, nickname , headimgurl, email, wx_account',
            $where
        );
