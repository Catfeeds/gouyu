...
class UserModel extends Model
{
  ...
  //  protected $tableName = 'users'; 
    ...
    public function saveUserInfo()
	{
		if (!empty($this->data))
		{
			//入库前过滤敏感字段
			$this->filterField();
			return $this->where('user_id = ' . $this->userId)->save($this->data);
		}
	}
	...
	{
		$filter_fields = array('left_money', 'consumed_money', 'pay_money', 'password');
		$data = $this->data;
  ...
  public function setUsersEnable($user_id, $is_enable=2)
  {
    ...
    public function getAgentListData($user_list)
	{
		foreach ($user_list AS $k => $v)
		{
			...
			//上级昵称
			$user_info = $this->getUserInfo('nickname', 'user_id = ' . $v['first_agent_id']);
  ...
  */
  ...
  public function getUsersByRankId($agent_rank_id)
  {
     ...
     */
	public function getLeftAndFrozenMoney()
	{
		//获取用户表中当前用户的预存款余额
		$where = $this->checkFieldValid();
		$user_info = $this->getUserInfo('left_money, frozen_money');
		return $user_info;
	}
	...
	public function setFrozenMoney($frozen_money)
	{
		...
		if (is_float($frozen_money))
		{
			//修改用户表中当前用户的预存款余额
			$where = $this->checkFieldValid();
			return $this->where($where)->save(array('frozen_money' => $frozen_money));
		}
  ...
  [8] => string(9) "700055661"
    ...
    public function setId()
    {
        $id = $this->max('id');
        $rand_num = rand(1, 10);
        $id = $id >=  100000 ? ($id+$rand_num) : 100000;
