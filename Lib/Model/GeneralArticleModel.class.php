...
class GeneralArticleModel extends ArticleBaseModel
{
    ...
    const HELP_CENTER = 1;
    const ABOUT_US    = 2;
    const RULE        = 3; //规则sort为3;

    const ROLL_RULE = 1; //Ｒoll规则
    const EXCHANGE_RULE = 2; //兑换规则
    const TREASURE_RULE = 3; //夺宝规则
    const TREASURE_OPEN_RULE = 4; //夺宝开奖规则
    const GUESS_RULE = 5; //竞猜规则
	...
	public function getGeneralArticleListPage($where = '', $rows = 15)
	{
		...
		$fields = $_table . '.article_id,' .$_table . '.rule_type,' . $_table . '.title,' . $_table . '.clickdot,' .
				$_table . '.addtime,' . $_table . '.serial,' . $_table . '.isuse,a_s.article_sort_name';
		$join = C('DB_PREFIX') . 'article_sort AS a_s ON a_s.article_sort_id=' . $_table . '.article_sort_id';
     ...
     * @date 2016-8-30
     */
    public function getSpecialArticleContent($where)
    {
        $article_id = $this->where($where)->getField('article_id');

        $content = $this->getArticleContents($article_id);
