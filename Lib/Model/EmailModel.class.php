...
class EmailModel extends Model 
{
    ...
    {
        $from = array(
            'mail_address' => $GLOBALS['config_info']['MAIL_ADDRESS'],
            'mail_loginname' => $GLOBALS['config_info']['MAIL_LOGINNAME'],
            'mail_smtp' => $GLOBALS['config_info']['MAIL_SMTP'],
            'mail_password' => $GLOBALS['config_info']['MAIL_PASSWORD'],
            'fromname' => $GLOBALS['config_info']['FROMNAME'],
        );
...
/*      $ConfigModel = new ConfigBaseModel();                                //用来获取当前系统的配置信息
  ...
  */     
        ...
        //$smtppass = '123456';
        /**测试用数据结束**/
        $smtpserver = $from['mail_smtp'];
        $smtpusermail = $from['mail_address'];
        $smtpuser = $from['mail_loginname'];
        $smtppass = $from['mail_password'];
        ...
        $send_state = $smtp->sendmail($smtpemailto, $smtpusermail, $mailsubject, $mailbody, $fromname, $mailtype);

        $arr = array(
            'send_email_account'=> $smtpusermail,
            'email_recipients'  => $smtpemailto,
            'subject'           => $mailsubject,
            'send_time'         => NOW_TIME,
            'state'             => $send_state,
        );
        $this->addEmaiSendlLog($arr);
