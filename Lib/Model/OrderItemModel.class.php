...
class OrderItemModel extends Model
{
	 ...
	 */
    public function OrderItemModel($order_id)
    {
		$this->db(0);
		$this->orderId = $order_id;
		$this->tableName = C('DB_PREFIX') . 'order_item';
	}
    ...
    public function getOrderItemList($fields = '', $where = '', $order = '')
    {
		...
		//从数据库中查询
		$where = $where ? $where : 'order_id = ' . $this->orderId;
    ...
    public function addItem($item_info, $agent_rank_id = 0)
    {
		...
		/*foreach ($item_info as $k => $v) {
			//取出套餐商品里单个商品
			$item_package_obj = new ItemPackageDetailModel();

			$fields = 'item_name, item_sn, weight, mall_price, base_pic';
			if($v['item_package_id'] != 0){
				$item_list[$k] = $item_package_obj->getPackageItemList($fields,'item_package_id = ' . $v['item_package_id']);
				$item_list[$k] = D('Item')->getListData($item_list[$k]);
			}
		}*/
		//用户等级求折扣
		...
		#echo "<pre>";echo $item_package_obj->getLastSql(); print_r($item_list);die;
		...
		foreach ($item_info AS $key => $item)
		{
			// if($item['item_id'] != 0){
				...
				//获取套餐商品信息
				if($item['item_id'] == 0){
					$fields = 'item_name, item_sn, mall_price, base_pic';
					$item_package_basic_info = $item_package_obj->getItemPackageInfo('item_package_id = ' . $item['item_package_id'], $fields);
            		$item_package_basic_info['vip_price'] = round($user_rank['discount'] * $item_package_basic_info['mall_price'] / 100, 2);

				}
				
				//获取商品信息
				$fields = 'item_name, item_sn, weight, mall_price, base_pic, goods_code';
				$item_basic_info = $item_obj->getItemInfo('item_id = ' . $item['item_id'], $fields);
            	$item_basic_info['vip_price'] = round($user_rank['discount'] * $item_basic_info['mall_price'] / 100, 2);

				#echo $item_obj->getLastSql();
				#echo "<pre>";print_r($item_basic_info);

				//获取商品货号
				$item_sn = '';
				if (!$item_sku_price_id)
				{
					$item_sn = $item_basic_info['item_sn'] ? $item_basic_info['item_sn'] : $item_package_basic_info['item_sn'];
				}
				else
				{
				...
				$item_info[$key]['order_id'] = $this->orderId;
				$item_info[$key]['item_sku_price_id'] = $item['item_sku_price_id'];
				$item_info[$key]['item_name'] = $item_basic_info['item_name'] ? $item_basic_info['item_name'] : $item_package_basic_info['item_name'];
				$item_info[$key]['weight'] = $item_basic_info['weight'] ? $item_basic_info['weight'] : $item_package_basic_info['weight'];
				$item_info[$key]['item_sn'] = $item_sn;
				$item_info[$key]['mall_price'] = floatval($item_basic_info['mall_price']) ? floatval($item_basic_info['mall_price']) : floatval($item_package_basic_info['mall_price']);
				$item_info[$key]['vip_price'] = floatval($item_basic_info['vip_price']) ? floatval($item_basic_info['vip_price']) : floatval($item_package_basic_info['vip_price']);
				$item_info[$key]['goods_code'] = $item_basic_info['goods_code'];
				if($item_basic_info['base_pic'])
				{
					$item_info[$key]['small_pic'] = $item_basic_info['base_pic'] ? $item_basic_info['base_pic'] : $item_package_basic_info['base_pic'];
				}
				
				$item_info[$key]['real_price'] = floatval($item_basic_info['vip_price']) ? floatval($item_basic_info['vip_price']) : floatval($item_package_basic_info['vip_price']);
				$item_info[$key]['cost_price'] = floatval($item_basic_info['cost_price']) ? floatval($item_basic_info['cost_price']) : floatval($item_package_basic_info['cost_price']);
				...
				$total_amount += floatval($item_basic_info['vip_price']) ? floatval($item_basic_info['vip_price']) : floatval($item_package_basic_info['vip_price']) * $item_info[$key]['number'];
				$cost_amount += floatval($item_basic_info['cost_price']) ? floatval($item_basic_info['cost_price']) : floatval($item_package_basic_info['cost_price']) * $item_info[$key]['number'];
		...
		// $this->addAll($item_list);
		#echo $this->getLastSql();echo "<pre>";print_r($item_info);die;
		#echo "</pre>";
		$return_arr = array(
    ...
    //设置数据
    public function setOrderItemData($arr)
    {
        if ($this->checkOrderIdvalid()) {
    ...
    public function saveOrderItem()
    {
        ...
        if ($this->add($this->data)) return $this->data;
    }
}
