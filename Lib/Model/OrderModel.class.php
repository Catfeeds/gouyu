...
class OrderModel extends Model
{
    ...
    public function batchAddItem($item_info)
    {
        ...
        $arr = array(
            ...
            'item_sn' => $item_info['item_sn'],
            'number' => $item_info['item_num'],
            'real_price' => $item_info['real_price'],
            'cost_price' => $item_info['cost_price'],
            'small_pic' => $item_info['base_pic'],
            'is_real' => $item_info['is_real'],
        );
        $order_item_obj->setOrderItemData($arr);
        $item_info = $order_item_obj->saveOrderItem();
	 ...
	 */
    public function addOrder($order_info, $item_info, $address_info)
    {
		//创建订单，将订单基本信息保存到数据库
        ...
        //地址
        trace($address_info);
        $order_address_obj = new OrderAddressModel($this->orderId);
        $address_info['user_id'] = intval(session('user_id'));
        $address_info['addtime'] = NOW_TIME;
        $address_info['isuse'] = 1;
        $order_address_obj->addOrderAddress($address_info);
	 ...
	 */
    public function deliverOrder($express_company_id=0,$express_number=0)
    {
    ...
    public function payOrderEvent()
    {
        ...
        $item_info = M('OrderItem')->where('order_id = ' . $this->orderId)->find();
        if (!$item_info)
        {
            throw new Exception('商品信息不存在');
        }
        //减库存
        $item_obj = new ItemModel($item_info['item_id']);
        $item_obj->deductItemStock($item_info['item_id'], $item_info['item_sku_price_id'], $item_info['number']);

        //设置销量
        //$item_obj->where('item_id = ' . $item_info['item_id'])->setInc('sales_num');
    }
	 ...
	 * @todo 发邮件，发短信等
	 */
    public function payOrderEvents()
    {
		//获取商品列表
...
log_file('useCoupon: sql = ' . $user_vouchers_obj->getLastSql(), 'useCoupon');
    ...
    public function getListData($order_list)
	{
		foreach ($order_list AS $k => $v)
		{
            ...
            $order_address_info = $order_address_obj->getOrderAddressInfos('order_id = ' . $v['order_id']);
            if ($v['need_deliever']) {
                # 获取快递公司名称
                ...
                # 获取收货人名字
                $province_id        = $order_address_info['province_id'];
                $city_id            = $order_address_info['city_id'];
                $area_id            = $order_address_info['area_id'];
                $address_detail     = $order_address_info['address'];

                $area_obj           = new AreaModel();
                $address            = $area_obj->getAreaString($province_id, $city_id, $area_id);

                $order_list[$k]['consignee']  = $order_address_info['realname'] ? $order_address_info['realname']:'--';
                $order_list[$k]['address']    = $address ? $address . $address_detail : '--';
                $order_list[$k]['mobile']     = $order_address_info['mobile']? $order_address_info['mobile'] : '--';
            } else {
                $order_list[$k]['game_name'] = M('Game')->where('game_id = ' . $order_address_info['game_id'])->getField('game_name');
                $order_list[$k]['game_account'] = $order_address_info['game_account'];
                $order_list[$k]['steam_url'] = $order_address_info['steam_url'];
            }

            //调用订单商品模型获取订单商品列表
            $order_item_obj = new OrderItemModel($v['order_id']);
            $item_list = $order_item_obj->getOrderItemList();
            $order_list[$k]['item_num'] = $item_list[0]['number'];
            $order_list[$k]['real_price'] = $item_list[0]['real_price'];
		}
    ...
    public function getFrontListData($order_list)
	{
		foreach ($order_list AS $k => $v)
		{
            ...
            //商品信息
            $item_info = M('OrderItem')->where('order_id = ' . $v['order_id'])->find();
            $order_list[$k]['item_id'] = $item_info['item_id'];
            $order_list[$k]['item_name'] = $item_info['item_name'];
            $order_list[$k]['small_pic'] = $item_info['small_pic'];
            $order_list[$k]['ajax_addtime'] = date('Y-m-d h:i:s', $v['addtime']);
		}
    ...
    protected function passRefundApplyConfirmdEvent()
    {
        ...
        if ($order_info) {
            ...
            $account_obj->addAccount($order_info['user_id'], AccountModel::ORDER_REFOUND, $order_info['pay_amount'], '用户申请退款', $this->orderId);
        }
    }
	...
	function gain()
	{
		...
		if ($user_info['first_agent_id'])
		{
			...
			$account_obj = new AccountModel();
			$account_obj->addAccount($user_info['first_agent_id'], AccountModel::FIRST_LEVEL_AGENT_RATE, $amount, '一级推荐用户消费提成');
