...
class WXPayModel extends PaywayModel 
{
	...
	public function __construct()
	{
		...
		$payway_info = $payway_obj->getPaywayInfoByTag('wxpay');
...
trace($row_dd, 'wuzeguo_wxpay');
		...
		elseif ($pay_type == 'voucher')
		{
			...
			if ($trade_status == 'SUCCESS')
			{
				...
				//调用账户模型的addAccount充值等额预存款金额
                $total_fee = intval($data['total_fee']) * $GLOBALS['config_info']['RMB_EXCHANGE_PROP_WXPAY'] / 100;
				$account_obj->addAccount($user_id, 1, $total_fee, '微支付充值', 0, $trade_no);
